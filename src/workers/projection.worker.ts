...
import { tSNE } from '../utils/algorithms/tSNE';
import { incrementalPCA } from '../utils/algorithms/incrementalPCA';
import { uMAP } from '../utils/algorithms/uMAP';

// Worker message handler
self.onmessage = (event: MessageEvent) => {
  const { embeddings, documents, options }: { embeddings: number[][], documents: Document[], options: ProjectTo2DOptions } = event.data;

  try {
    const result = projectTo2D(embeddings, documents, options);
